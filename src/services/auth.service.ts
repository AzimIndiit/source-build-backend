...
class AuthService {
  ...
  ): Promise<{ user: IUser; tokens: IAuthTokens; otpSent: boolean }> {
    try {
      ...
      // Log if there's a deleted account with this email (for audit purposes)
      const deletedAccount = await UserModal.findOne({
        email: registerData.email.toLowerCase(),
        status: UserStatus.DELETED,
      })

      if (deletedAccount) {
        logger.info('New registration with email of previously deleted account', {
          email: registerData.email,
          previousUserId: deletedAccount._id,
        })
      }
  ...
  async login(credentials: ILoginCredentials): Promise<{ user: IUser; tokens: IAuthTokens }> {
    try {
      logger.info('Login attempt', { email: credentials.email ,role: credentials.role})
console.log('credentials', credentials)
      // Build query based on whether admin role is specified
      const query: any = {
        email: credentials.email.toLowerCase(),
      }
      
      // Only include role in query if it's admin
      // This ensures admin accounts are role-specific
      ...
      if (user.status === UserStatus.INACTIVE) {
        throw ApiError.forbidden(
          'Your account is deactivated. Please contact support at support@sourcebuild.com'
        )
      }

      // Check if account is deleted
      if (user.status === UserStatus.DELETED) {
        throw ApiError.forbidden('Account not found. Please register first!')
      }
